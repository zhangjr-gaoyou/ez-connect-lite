		...
		int endpoint, int authmode)
{
	...
	/**
	 * Send max fragment size in extension during ssl handshake.
	 */
...
#elif (MBEDTLS_SSL_MAX_CONTENT_LEN == 512)
	mbedtls_ssl_conf_max_frag_len(conf, MBEDTLS_SSL_MAX_FRAG_LEN_512);
#elif (MBEDTLS_SSL_MAX_CONTENT_LEN == 16384)
	/**
	 * Note:
	 * In this case do nothing, since 16k (16384) is standard frangment
	 * length.
	 */
#else
  #error "Invalid MBEDTLS_SSL_MAX_CONTENT_LEN"
#endif
