...
def copy_config_option(comp_name, comp_file):
    if (comp_name != 'debug') and (os.path.exists(comp_file) == False):
        print "File " + comp_file + " does not exist"
        cleanup()
        sys.exit()
    with open (CONFIG_FILE, 'a') as cfile:
        cfile.write(comp_name + ' ' + comp_file + '\n')
...
def copy_mfg_option(mfg_cmd, arg, no_subargs):
    try:
        ...
        try:
            ...
            if (mfg_cmd == 'write'):
                if (os.path.exists(subargs[2]) == False):
        ...
        if (mfg_cmd == 'read'):
            line = 'read ' + subargs[0] + ' ' + subargs[1] + ' ' + subargs[2] + ' ' + subargs[3]
        elif (mfg_cmd == 'write'):
            line = 'write ' + subargs[0] + ' ' + subargs[1] + ' ' + subargs[2]
        elif (mfg_cmd == 'erase'):
...
def main():
    ...
    non_comp_longopts = ["interface=","new-layout=","debug","batch=","read=","write=","erase=","erase-all=","prod=","reset","interactive","help","--interface"]
